...
(function() {
  var Q, XHRDownloadStream, bytesToMB, checkPath, config, db, diskspace, fs, getJson, getToDisk, i18n, os, path, post, printablePath, qs, url, workingFile;
  ...
  printablePath = function(pathToTest) {
    if (os.platform() === 'win32') {
      return pathToTest[0] + ":";
    } else {
      return path.dirname(pathToTest);
  ...
  getToDisk = function(targetUrl, opts) {
    ...
    finalDir = opts.finalDir || db.fetchObj(config.DbKeys.AppInstallDir);
