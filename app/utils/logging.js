...
(function() {
  var config, fs, getLogger, isProduction, log, logStream, os, path, pathToLog, saveLogIfNeeded;
  ...
  saveLogIfNeeded = function(pathToLog) {
    if (/^(WARN|ERROR):/g.test(fs.readFileSync(pathToLog))) {
      return fs.renameSync(pathToLog, pathToLog + '.' + Date.now());
    }
  };
