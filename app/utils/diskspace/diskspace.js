...
function check(drive, callback)
{
	...
	{
		child_process.execFile(path.join(__dirname, 'drivespace.exe'), ['drive-' + drive], function(error, stdout, stderr)
		{
			if (error)
			{
				status = 'STDERR';
	...
	{
		child_process.execFile("df", ['-k', drive], function(error, stdout, stderr) {
			if (error)
			{
